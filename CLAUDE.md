...
cargo test                         # 기본 테스트 (125개)
cargo test --features embed        # 전체 테스트 (131개, 임베딩 포함)

# 개별 테스트
cargo test --test personality_test    # HEXACO 성격 모델 (18개)
cargo test --test emotion_test        # OCC 감정 + Relationship 통합 (27개)
...
tests/
  ...
  emotion_test.rs                 # 감정 + Relationship 통합 (27개)
...
docs/                              # 설계 문서 (한국어)
  ...
  HEXACO/                         # HEXACO 6차원 상세 가이드
    h-dimension-guide.md          # 정직-겸손 (Honesty-Humility)
    e-dimension-guide.md          # 정서성 (Emotionality)
    x-dimension-guide.md          # 외향성 (Extraversion)
    a-dimension-guide.md          # 원만성 (Agreeableness)
    c-dimension-guide.md          # 성실성 (Conscientiousness)
    o-dimension-guide.md          # 경험개방성 (Openness)
.cargo/
...
- `docs/HEXACO/` — HEXACO 6차원별 상세 가이드 (h, e, x, a, c, o)
