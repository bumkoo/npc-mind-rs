...
│             ▼              ▼                             │
│  ┌──────────────┐  ┌─────────────────┐                  │
│  │MindRepository│  │ConversationPort │ ← 새로운 포트     │
│  │(기존)        │  │                 │                   │
│  └──────────────┘  └────────┬────────┘                  │
│                             │                            │
├─────────────────────────────┼────────────────────────────┤
...
src/
  adapter/
    rig_chat.rs            ← NEW: ConversationPort 구현 (rig-core)
  application/
    dialogue_test_service.rs  ← NEW: 대화 테스트 오케스트레이터
  ports.rs                 ← MODIFY: ConversationPort 추가
  bin/mind-studio/
    handlers.rs            ← MODIFY: /api/chat/* 엔드포인트 추가
    state.rs               ← MODIFY: DialogueTestService 상태 추가
    static/index.html      ← MODIFY: 채팅 UI 패널 추가
tests/
  dialogue_test.rs         ← NEW: 대화 루프 통합 테스트
  chat_adapter_test.rs     ← NEW: RigChatAdapter 단위 테스트
mcp/
  mind_studio_server.py    ← MODIFY: chat 관련 MCP 도구 추가
```
