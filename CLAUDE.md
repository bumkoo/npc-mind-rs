...
- `CommandDispatcher<R>` + `ConversationPort` 조합으로 Event Sourcing 경로에 맞춘 다턴 대화
- `::new(dispatcher, chat, formatter)` / `.with_analyzer(analyzer)`
- `start_session()` — Appraise → EmotionAppraised 이벤트 + LLM 세션 시작
- `turn(utterance, pad?)` — DialogueTurnCompleted(user) → ApplyStimulus → [BeatTransitioned 시 update_system_prompt] → LLM 호출 → DialogueTurnCompleted(assistant) 이벤트 발행
- `end_session(significance?)` — LLM 세션 종료 + (선택) EndDialogue 커맨드
...
├─ DialogueAgent (orchestrator) [chat] ───────────────────────┤
│  start_session / turn / end_session async API               │
│  dispatcher 경유로 Appraise·ApplyStimulus·EndDialogue 발행  │
│  DialogueTurnCompleted(user/assistant) 이벤트 직접 발행     │
