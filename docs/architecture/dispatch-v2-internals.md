...
dispatch_v2(cmd).await
  ...
  │
  ├─ ⑤ commit_staging_buffer(..) → Vec<DomainEvent>
  │     id/seq 할당 + cid·cascade_depth·parent_event_id 부착 → 단일 EventStore.append
  │
...
```rust
// 큐 element: (depth, event, parent_staging_idx).
// parent_staging_idx == None → 초기 커맨드 이벤트 (트리 root).
...
let mut parent_indices: Vec<Option<usize>>   = Vec::new();   // staging_buffer 와 동기 누적
...
while let Some((depth, event, parent_idx)) = event_queue.pop_front() {
    ...
    let my_idx = staging_buffer.len();   // follow-up 자식들이 부모로 가리킬 인덱스
    ...
    for handler in self.transactional_handlers.iter() {      // ← priority 오름차순 사전정렬
        ...
        if can_emit_follow_up {
            for fu in result.follow_up_events {
                event_queue.push_back((depth + 1, fu, Some(my_idx)));
            }
        }
    }
...
- **인과 트리 추적**: 큐 element 의 세 번째 요소가 staging_buffer 내 부모 인덱스. 부모의 EventStore id 는 commit 전이라 미정이므로 인덱스로 가리키고, ⑤에서 id 매핑한다. **단일 스레드 BFS 가정** — 핸들러 병렬화 도입 시 토큰 기반으로 전환 필요.
...
// 자식 차례엔 committed[parent_idx] 가 이미 id 를 받은 상태. 단일 패스로 충분.
...
self.event_store.append(&committed);    // 단일 batch append
```
...
**단일 batch append**: 이벤트마다 즉시 `append(&[e.clone()])` 하지 않고 metadata 가 완전히 채워진 뒤 한 번에 `append(&committed)` 한다. metadata 미완성 상태가 EventStore 외부 구독자에 노출되지 않도록. `InMemoryEventStore::append` 는 단일 lock 안의 `extend` 라 원자적. 영속 백엔드 도입 시엔 트랜잭션이 필요하다.
...
7. **`correlation_id` 는 `dispatch_v2` 진입 시 자동 발급**되며 함수 로컬 변수다. 외부 주입용 `set_correlation_id` 는 `#[deprecated]` (no-op). 동시 dispatch 안전.
...
9. **`parent_event_id` 인덱스 추적은 단일 스레드 BFS 가정.** 핸들러 병렬화를 도입하면 `staging_buffer` 인덱스 안정성이 깨지므로 토큰 기반으로 전환해야 한다.
