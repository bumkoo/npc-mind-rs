...
| 배율 | 공식 | 적용 감정 |
|------|------|----------|
| `rel_mul` | `(1.0 + closeness × REL_CLOSENESS_INTENSITY_WEIGHT).max(0.0)` | **Admiration, Reproach에만 적용** |
| `trust_mod` | `1.0 + trust × REL_TRUST_EMOTION_WEIGHT` | **Admiration, Reproach에만 적용** |
| `empathy_rel_modifier` | `(1.0 + closeness × REL_CLOSENESS_EMPATHY_WEIGHT).max(0.0)` | HappyFor, Pity |
| `hostility_rel_modifier` | `(1.0 - closeness × REL_CLOSENESS_HOSTILITY_WEIGHT).max(0.0)` | Resentment, Gloating |
...
| 0.4.0 | 2026-03-30 | 관계 배율 매직 넘버→tuning.rs 상수명 반영 (REL_CLOSENESS_INTENSITY_WEIGHT 등) |
