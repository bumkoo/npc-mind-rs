...
- **Libraries:** `serde`/`serde_json`, `thiserror`, `axum`/`tokio`(WebUI), `tracing`(Appraisal Trace), `ort`(ONNX 임베딩), `rig-core`(LLM Agent 대화)

## 빌드 & 테스트

```bash
cargo build                        # 기본 빌드 (임베딩/대화 제외)
cargo build --features embed       # 임베딩 포함 빌드 (bge-m3-onnx-rust)
cargo build --features chat        # LLM 대화 에이전트 포함 빌드 (rig-core)
cargo test                         # 기본 테스트
...
cargo run --features mind-studio,chat --bin npc-mind-studio     # LLM 대화 테스트 포함
cargo run --features mind-studio,chat,embed --bin npc-mind-studio  # 전체 기능 (PAD 자동 분석 + 대화)

# 환경변수
# NPC_MIND_CHAT_URL=http://127.0.0.1:8081/v1   (로컬 추론 서버 URL, 기본값) [chat feature]
# NPC_MIND_CHAT_MODEL=local-model               (모델 이름, 기본값) [chat feature]
# MIND_STUDIO_HOST=127.0.0.1                    (서버 호스트, 기본값) [mind-studio feature]
# MIND_STUDIO_PORT=3000                         (서버 포트, 기본값) [mind-studio feature]
# NPC_MIND_MODEL_DIR=../models/bge-m3           (ONNX 모델 디렉토리, 기본값) [embed feature]
# NPC_MIND_ANCHOR_LANG=ko                       (PAD 앵커 언어, 기본값) [embed feature]
```
...
`--features chat` 사용 시 rig-core의 TLS 백엔드 주의:
- Windows MSVC 환경에서 기본 `aws-lc-sys`(rustls)는 `__builtin_bswap` GCC intrinsic 때문에 링크 실패함.
- 해결: Cargo.toml에서 `rig-core`를 `default-features = false, features = ["reqwest-native-tls"]`로 설정.
- rig 0.33부터 OpenAI provider가 기본적으로 Responses API(`/v1/responses`)를 사용함.
  llama.cpp 등 로컬 서버는 Chat Completions API만 지원하므로 `.completions_api()` 호출 필수.
...
src/
  application/                    # 어플리케이션 계층 (라이브러리 진입점)
    mind_service.rs               # MindService — 퍼사드 및 핵심 오케스트레이션
    relationship_service.rs       # [NEW] RelationshipService — 관계 갱신 전담
    scene_service.rs              # [NEW] SceneService — 장면 및 비트 전환 전담
    situation_service.rs          # [NEW] SituationService — DTO → 도메인 변환 전담
    formatted_service.rs          # FormattedMindService — MindService + 포맷터 조합
    dto.rs                        # API 데이터 전송 객체 (CanFormat 트레이트 도입)
    dialogue_test_service.rs      # DialogueTestService — 대화 테스트 오케스트레이터 (chat feature)
  domain/
  ...
  adapter/                        # 포트 구현 (인프라 어댑터)
    ...
    rig_chat.rs                   # rig-core 기반 LLM 대화 어댑터 (chat feature)
    file_anchor_source.rs         # [MERGED] JSON/TOML 통합 PAD 앵커 소스 구현
    anchor_common.rs              # PAD 앵커 소스 공통 유틸리티
  ...
  bin/mind-studio/                # NPC Mind Studio (Axum 서버)
    ...
    static/index.html             # 웹 UI (LLM Model 탭 포함)
tests/
...
locales/
  ...
  anchors/
    ...
    ko.embeddings.json             # 사전 계산된 PAD 앵커 임베딩 캐시
docs/
  api/
  ...
  architecture/
    ...
    adr-001-rig-agent-integration.md # ADR: rig-core LLM Agent 통합 (Accepted)
  ...
  assets/
    mind-studio-preview.png         # Mind Studio 미리보기 이미지
  collaboration-workflow.md         # 협업 워크플로우
  locale-guide.md                   # 언어 설정 가이드 (개발자/사용자)
  tech-debt-audit-2026-04.md        # 기술 부채 감사 (2026-04)
data/
  huckleberry_finn/                 # 소설 기반 테스트 시나리오
    ...
    ch15_fog_trash/                 # 15장: 안개 속 쓰레기
      session_001~004/              # scenario.json + evaluation + turn logs (4개 세션)
  presets/                          # 캐릭터 사전설정 JSON (so_ho, shu_lien, mu_baek, gyo_ryong)
mcp/
  mind_studio_server.py             # MCP 서버 — Mind Studio API 26개 도구 노출 (Python + mcp SDK)
...
| `PadAnchorSource` | PAD 앵커 로드/캐시 | `FileAnchorSource` (JSON/TOML 통합) |
| `TextEmbedder` | 텍스트 → 벡터 | `OrtEmbedder` (embed feature) |
| `UtteranceAnalyzer` | 대사 → PAD | `PadAnalyzer` |
| `ConversationPort` | LLM 다턴 대화 세션 (chat feature) | `RigChatAdapter` (rig-core 0.33 + `openai::CompletionsClient`) |
...
### 데이터 변환 (Mapping)
- DTO(`SituationInput` 등)는 `SituationService`를 통해 도메인 모델로 변환됩니다.
- DTO 자체는 저장소 의존성이 없는 순수 데이터 구조체로 유지되며, 서비스가 저장소(`MindRepository`)에서 필요한 관계/오브젝트 정보를 조회하여 변환 시 주입합니다.
- **CanFormat 트레이트**: `AppraiseResult`, `StimulusResult` 등 도메인 결과 타입은 `CanFormat` 트레이트를 구현하여 포맷팅된 `Response`로의 변환을 표준화합니다.
...
**대화 테스트 (chat feature):**
- `POST /api/chat/start` — 대화 세션 시작: `{session_id, appraise: AppraiseRequest}` → 감정 평가 → 프롬프트를 system prompt로 rig Agent 생성
- `POST /api/chat/message` — 대사 전송: `{session_id, npc_id, partner_id, utterance, pad}` → LLM 응답 + PAD stimulus → 감정 변화 + Beat 전환 시 system_prompt 자동 갱신
- `POST /api/chat/message/stream` — 대사 전송 (스트리밍): 토큰 단위 SSE 스트리밍 응답
- `POST /api/chat/end` — 세션 종료: `{session_id, npc_id, partner_id, significance}` → 대화 이력 반환 + 관계 갱신
...
- `POST /api/save` — 현재 상태 저장 (시나리오 또는 결과)
- `GET /api/save-dir` — 결과 저장 디렉토리 경로 조회
- `POST /api/load` — 시나리오 로드 (scene 필드 시 Focus 자동 등록)
- `POST /api/load-result` — 테스트 결과 로드 (히스토리 포함)
...
Mind Engine이 생성한 프롬프트를 실제 LLM에 system prompt로 주입하고, 다턴 대화를 통해 NPC 연기 품질을 검증하는 기능.

### 아키텍처
- **ConversationPort** (`ports.rs`): LLM 대화 세션 추상화 — `start_session`, `send_message`, `send_message_stream`, `update_system_prompt`, `end_session`
- **RigChatAdapter** (`adapter/rig_chat.rs`): rig-core 0.33 `openai::CompletionsClient` 기반 구현
  - `openai::Client::builder().api_key().base_url().build().completions_api()` 로 생성
  - `Chat::chat(&agent, user_message, history)` 로 다턴 대화 수행
  - 세션별 system_prompt + rig_history + dialogue_history 관리
- **DialogueTestService** (`application/dialogue_test_service.rs`): `FormattedMindService` + `ConversationPort` 오케스트레이터
  - `start_chat()`: appraise → 프롬프트 생성 → LLM 세션 시작
  - `chat_turn()`: LLM 응답 → PAD stimulus → 감정 변화 → Beat 전환 시 system_prompt 갱신
  - `end_chat()`: 세션 종료 → 관계 갱신 → 대화 이력 반환

### 대화 루프 핵심 흐름
```
1. appraise() → prompt 생성
2. ConversationPort.start_session(prompt)
3. 반복:
   a. ConversationPort.send_message(상대 대사) → NPC 응답
   b. apply_stimulus(PAD) → 감정 변화
   c. Beat 전환 시: update_system_prompt(새 프롬프트)
4. ConversationPort.end_session() → 대화 이력
5. after_dialogue() → 관계 갱신
```

### Cargo.toml 의존성 (chat feature)
```toml
...
| LLM_BASE_TEMPERATURE | 0.8 | 기본 Temperature |
| LLM_TEMP_MAX | 1.1 | Temperature 최대 임계값 |
| LLM_TOP_P_MIN | 0.8 | Top P 최소 임계값 |
