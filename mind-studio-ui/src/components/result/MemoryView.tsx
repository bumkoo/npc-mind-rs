...
/**
 * 기억 탭 — NPC별 / Topic별 모드를 토글한다.
 *
 * 부모는 모드 전환만 담당. NPC 모드의 상태·효과는 `NpcModeView`, Topic 모드는
 * `TopicHistoryView`가 각각 자체 훅을 소유. 덕분에 비활성 모드의 fetch·
 * `loading` 교차 오염이 발생하지 않는다 (E3.1 리뷰 C1+H1).
 */
...
/**
 * NPC별 기억 뷰. 활성일 때만 마운트되므로 관련 effect·state가 Topic 모드와
 * 격리된다. `/api/memory/by-npc/:id` 결과를 표시 (Step E2).
 */
function NpcModeView({ npcs }: NpcModeProps) {
  ...
  // M1: 마운트 / 사이드바 NPC 변경 시 선택이 없으면 UI store의 npcId를 기본값으로 복사.
  // 명시 선택은 덮어쓰지 않는다.
  useEffect(() => {
    if (!selectedNpcId && uiNpcId) {
      setSelectedNpcId(uiNpcId)
    }
    // 의존성: uiNpcId만 — selectedNpcId를 의존에 넣으면 명시 해제 직후 재복사되는 루프 발생.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [uiNpcId])

  // NPC 선택 시 targeted fetch (H1 flash 방지 + H2 AbortController).
  useEffect(() => {
    if (!selectedNpcId) {
      setEntries([])
      setLoading(false)
