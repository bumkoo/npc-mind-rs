...
src/
  application/    어플리케이션 계층, 라이브러리 진입점
                  ...
                  - event_service.rs (EventAwareMindService — **deprecated v1, v0.3.0 제거**)
                  - event_store.rs, event_bus.rs (Event Sourcing 인프라)
                  - pipeline.rs (**deprecated v1, v0.3.0 제거** — v2 dispatch_v2 transactional chain으로 대체)
                  - projection.rs (**trait/Registry deprecated v1** — 구조체는 v2 wrapper가 재사용)
                  ...
                  - command/ (CQRS Command Side — v1 + v2 공존)
                    - types.rs (Command enum + aggregate_key)
                    - handler.rs (HandlerContext/Output — **deprecated v1**)
                    - handler_v2.rs (EventHandler trait, EventHandlerContext, HandlerShared + test_support::HandlerTestHarness)
                    - priority.rs (SCENE_START/EMOTION_APPRAISAL/STIMULUS_APPLICATION/GUIDE_GENERATION/RELATIONSHIP_UPDATE 상수 + invariants)
                    - dispatcher.rs (CommandDispatcher: v1 dispatch + **v2 dispatch_v2 + with_default_handlers**)
                    - projection_handlers.rs (B안 B2 — EmotionProjectionHandler/RelationshipProjectionHandler/SceneProjectionHandler Inline EventHandler wrappers)
                    - agents/ (v1 handle_* + v2 impl EventHandler 공존)
                      - emotion_agent.rs (AppraiseRequested → EmotionAppraised)
                      - stimulus_agent.rs (B1 신규 — StimulusApplyRequested → StimulusApplied/BeatTransitioned)
                      ...
                      - scene_agent.rs (B4.1 신규 — SceneStartRequested → SceneStarted + EmotionAppraised)
  ...
  domain/         순수 도메인 로직
                  ...
                  - event.rs (DomainEvent, EventPayload — 15 variants 포함 *Requested 6종, Event Sourcing)
  ...
  bin/mind-studio/  Axum REST API + SSE MCP 서버 + SSE 실시간 동기화 + static 파일 서빙
                  - /api/*       v1 경로 (MindService 기반)
                  - /api/v2/*    v2 shadow 경로 (B4 S3 B-Mini, Director 경유, 분리 Repository)
tests/            통합 테스트 (TestContext 공유) — command_test/event_test/pipeline_test(v1) + dispatch_v2_test/director_test(v2)
...
**`EventAwareMindService<R, A, S>`** — ⚠️ **deprecated (v0.2.0), v0.3.0 제거 예정** (`application/event_service.rs`)
- `MindService`를 감싸서 동일 API 유지 + 모든 호출에 `DomainEvent` 발행
- v2 대체: `CommandDispatcher::dispatch_v2` + `with_default_handlers()` 조합
...
**`Director<R>`** — 다중 Scene facade (`application/director/mod.rs`, B안 B4 Session 2)
- `::new(dispatcher)` — CommandDispatcher를 감싸며 Scene lifecycle 관리
- `start_scene(npc, partner, significance, focuses)` → `Command::StartScene` dispatch_v2
- `end_scene(scene_id, significance)` → `Command::EndDialogue` dispatch_v2
- `dispatch_to(scene_id, cmd)` — scene_id 검증 + v2 dispatch
- `active_scenes()` / `is_active(scene_id)` — 활성 Scene 목록
- `DirectorError::SceneNotActive / SceneAlreadyActive / SceneMismatch` — lifecycle 에러 variant
...
- `start_session(sid, npc, partner, situation?)` — `Command::Appraise` dispatch + LLM 세션 시작
- `turn(sid, utterance, pad?, sit_desc?)` — user 턴 이벤트 → `Command::ApplyStimulus` dispatch → (Beat 전환 시 `update_system_prompt`) → `send_message` → assistant 턴 이벤트
- `end_session(sid, significance?)` — LLM 세션 종료 + (significance 있으면) `Command::EndDialogue` dispatch
...
B안(v2) 이행 완료. v1은 B5 단계별 deprecation 진행 중 (v0.3.0 제거 예정).
...
### DomainEvent (15 variants)

#### v2 초기 이벤트 (6종 *Requested — Command → initial event)
...
- `Npc(npc_id)`: 그 외 (AppraiseRequested/EmotionAppraised/StimulusApply(Requested)/GuideRequested/GuideGenerated/DialogueTurnCompleted/EmotionCleared)

### Pipeline (⚠️ deprecated v0.2.0, v0.3.0 제거)

v2 `dispatch_v2`의 transactional handler chain (BFS + follow_up_events)으로 대체됨.
기존 사용자는 `with_default_handlers()` + `dispatch_v2(cmd)`로 마이그레이션.
