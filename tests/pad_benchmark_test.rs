...
fn pad_분석기_벤치마크() {
    ...
    for case in &cases {
        ...
        let p_ok = if case.expected_p > 0.0 { p_total += 1; pad.pleasure > 0.0 }
                   else if case.expected_p < 0.0 { p_total += 1; pad.pleasure < 0.0 }
                   else { true };
        let a_ok = if case.expected_a > 0.0 { a_total += 1; pad.arousal > 0.0 }
                   else if case.expected_a < 0.0 { a_total += 1; pad.arousal < 0.0 }
                   else { true };
        let d_ok = if case.expected_d > 0.0 { d_total += 1; pad.dominance > 0.0 }
                   else if case.expected_d < 0.0 { d_total += 1; pad.dominance < 0.0 }
                   else { true };
        ...
        let p_mark = if case.expected_p != 0.0 { if p_ok { "✓" } else { "✗" } } else { " " };
        let a_mark = if case.expected_a != 0.0 { if a_ok { "✓" } else { "✗" } } else { " " };
        let d_mark = if case.expected_d != 0.0 { if d_ok { "✓" } else { "✗" } } else { " " };

        println!("{:<16} {:>+7.3}{} {:>+7.3}{} {:>+7.3}{}  {}",
            case.label,
            pad.pleasure, p_mark,
            pad.arousal, a_mark,
            pad.dominance, d_mark,
            case.utterance
        );
    }

    println!("{}", "-".repeat(90));
    println!("방향 정확도:  P={}/{} ({:.0}%)  A={}/{} ({:.0}%)  D={}/{} ({:.0}%)",
        p_correct, p_total, p_correct as f64 / p_total as f64 * 100.0,
        a_correct, a_total, a_correct as f64 / a_total as f64 * 100.0,
        d_correct, d_total, d_correct as f64 / d_total as f64 * 100.0,
    );
}
