...
**feature flag**: `listener_perspective` (Step 5에서 **default-on** 전환). 도메인 단위 테스트 **39개** + DialogueAgent 통합 **4개** 전수 통과.

**회귀 감시**: 기존 4개 벤치 수치 유지 확인 완료 (sign 81%, magnitude classifier 77%, magnitude 변환식 85%, prefilter_unit 10/10).

**Step 4 결과**: `DialogueAgent.turn()`에 옵셔널 Converter 주입 완료. `analyze_with_embedding`으로 임베딩을 PadAnalyzer와 공유, 변환 실패는 speaker PAD fallback + warn.
...
통합 지점:
- `application/dialogue_agent.rs::DialogueAgent::turn` — `Command::ApplyStimulus` dispatch 직전
- `bin/mind-studio/studio_service.rs::resolve_pad` → `convert_to_listener_pad` — `MindService::apply_stimulus` 호출 직전

`apply_stimulus(StimulusRequest)` / `Command::ApplyStimulus` / `EmotionAgent::handle_stimulus`는 utterance를 받지 않으므로 변환은 모두 PAD 결정 직후에 수행한다 (utterance + speaker_pad 둘 다 가용한 유일 지점).
...
1. `application/dialogue_agent.rs`: `Option<Arc<dyn ListenerPerspectiveConverter>>` 필드 + `with_converter()` 빌더
...
**주요 결정**: apply_stimulus(StimulusRequest)는 utterance를 받지 않아 MindService/EmotionAgent 안에서 변환 불가. utterance + speaker_pad가 모두 가용한 유일 지점인 DialogueAgent.turn()이 통합 위치.
...
**Must**
- `src/bin/mind-studio/studio_service.rs` `mod tests`: `resolve_pad`의 4-시나리오 단위 테스트 (DialogueAgent와 path-for-path drift 방지)
...
| 4 | DialogueAgent 통합 테스트 (4 시나리오) + 기존 dialogue_agent_test 9개 무회귀 |
...
| `tests/dialogue_converter_integration.rs` | 6 | DialogueAgent 통합: (a) 변환 / (b) 미주입 / (c) pad_hint skip / (d) 실패 fallback / (e) no-input silent skip / (f) 실제 EmbeddedConverter prefilter 경로 |
| `tests/dialogue_no_lp_passthrough.rs` | 1 | `--no-default-features --features chat` 회귀 감시 |
| `src/bin/mind-studio/studio_service.rs::tests` | 4 | Mind Studio `resolve_pad`의 같은 4-시나리오 (DialogueAgent와 path-for-path 검증) |
...
| 4. DialogueAgent 통합 | 1일 | **완료** | 중간 — `analyze_with_embedding` trait default 도입으로 임베딩 공유 |
