...
- **Architecture:** Hexagonal + DDD + **EventBus/CQRS/Event Sourcing** + **Multi-Agent**
- **Libraries:** `serde`/`serde_json`, `thiserror`, `axum`/`tokio`(WebUI), `tracing`, `ort`(ONNX 임베딩), `rig-core`(LLM Agent 대화), `rusqlite`(RAG 저장소 [embed])
...
| `MemoryStore` | RAG 기억 저장/검색 | `InMemoryMemoryStore`, `SqliteMemoryStore` [embed] |
...
├─ TieredEventBus ────────────────────────────────────────────┤
│  Tier 1 (동기): Projection 갱신, 즉시 완료 작업              │
│  Tier 2 (비동기): StdThreadSink/TokioSink, 백그라운드 처리   │
├─ MemoryAgent (EventBus subscriber) [embed] ─────────────────┤
...
Command 수신
  ...
  → EventBus.publish() (동기 콜백)
  → TieredEventBus.publish() (Tier 1 동기 + Tier 2 비동기)
```
...
| Tier | 실행 | 용도 | 구현 |
|------|------|------|------|
| Tier 1 (동기) | `publish()` 중 즉시 | Projection 갱신 | `subscribe_sync(Fn)` |
| Tier 2 (비동기) | 채널 → 백그라운드 | MemoryAgent 인덱싱, 향후 LLM 호출 | `StdThreadSink` / `TokioSink` [chat] |
...
저장소:
  InMemoryMemoryStore — Vec + brute-force cosine (테스트용)
  SqliteMemoryStore   — FTS5 텍스트 검색 + 벡터 BLOB (embed feature)
```
...
| Pipeline | ✅ 완료 | Pipeline (순차 체인), TieredEventBus (Sync+Async 2-Tier), StdThreadSink |
