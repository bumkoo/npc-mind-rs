...
> 상세 22감정 × HEXACO 매핑 표는 [hexaco-occ-emotion-mapping.md](hexaco-occ-emotion-mapping.md) 참조.
> 차원별 facet 상세는 [personality/facets/](../personality/facets/) 하위 문서 참조.

### 가중치 공식
모든 성격 가중치는 `Score::modifier(weight)` = `(1.0 + value × weight).max(0.0)` 패턴으로 적용된다.
...
### StimulusEngine 로직
1. **PAD 자극 공식**: `(P×P + A×A) × (1 + |D_n - D_o| × 0.3)`. P·A 축이 방향을 결정하고, D축 격차가 강도를 스케일링한다. (이전 단순 내적 방식에서 변경됨)
2. **방향성 적용**:
