...
4. dialogue_turn × N          상대 대사 입력 → LLM 연기 응답 + 감정 갱신
                              ※ 대사는 순수 발화만 (원칙 3)
5. dialogue_end               (선택) 세션 종료
6. after_dialogue             관계 갱신 (closeness/trust 수치 변동)
7. save_scenario(type="all")  result JSON + report MD 동시 저장
8. update_test_report         분석 레포트 작성 (한국어, 원칙 1)
```
