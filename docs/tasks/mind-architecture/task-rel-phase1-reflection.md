...
| 4 | **Stage 1 직전** — `Npc::inner_compass` + `compass_short_label()` 신설 (~30 LoC, **분리 commit**). 기존 NPC 생성 callsite는 `Option` 기본값으로 자동 호환 — **✅ 완료 (2026-05-11): commit `c3b3e21` (분리). Npc 5필드 / NpcBuilder.with_inner_compass / NpcJson serde(default) / 단위 테스트 5개. 회귀 1068 passed / 0 failed / 7 ignored** | A-min 인프라 완료 |
...
5. **Windows UAC installer detection heuristic** (2026-05-11 A-min 회귀 시 발견) — Windows의 *Installer Detection* 휴리스틱이 파일명에 `setup`/`install`/`patch`/`update`/`test` 등 키워드 포함 시 .exe를 *Installer*로 추정하고 elevation 요구. cargo의 `dispatch_v2_test.exe` (이름에 "test" 포함)가 일관되게 OS error 740 (`ERROR_ELEVATION_REQUIRED`) 반환. 같은 디렉토리의 `embed_test.exe` 등 다른 *_test.exe들은 영향 없음 — 발생 조건이 random하거나 Windows Defender의 reputation cache 의존. **회피**: `$env:__COMPAT_LAYER = "RunAsInvoker"` 환경변수 설정 (process compatibility shim) 후 cargo test 재실행. **장기 fix 권장**: `embed-resource` crate로 .exe에 `<requestedExecutionLevel level="asInvoker"/>` manifest 첨부 (Phase 1 범위 외, 후속 환경 task).
...
| v0.6 | 2026-05-11 | **F10 §4 A-min 분리 commit 완료** — `c3b3e21` `feat(domain): Npc.inner_compass + compass_short_label() (Phase 1 A-min)`. `src/domain/personality.rs`에 `Npc.inner_compass: Option<String>` 필드 + `inner_compass()` getter + `compass_short_label()` (Phase 1 alias) + `NpcBuilder::with_inner_compass()` 빌더 메서드. `src/adapter/memory_repository.rs`의 `NpcJson`에 `#[serde(default)] inner_compass: Option<String>` (시나리오 JSON 호환). 단위 테스트 5개 (default None / builder 설정 / Phase 1 alias / serde roundtrip / legacy JSON 호환). 회귀: **1068 passed / 0 failed / 7 ignored** (이전 baseline 1033 + dispatch_v2_test 30 정상 + 신규 5 = 1068). **신규 환경 이슈 1건 발견 — F11에 §5 추가**: Windows installer detection heuristic이 `dispatch_v2_test.exe`를 elevation 요구로 마킹 (OS error 740). 회피: `__COMPAT_LAYER=RunAsInvoker` 환경변수. F10 §4 ✅ 완료. **Stage 1 진입 가능**. |
| v0.7 | 2026-05-11 | **Stage 1 (Domain) 완료** — commit `891cc9a` `feat(domain): Phase 1 Stage 1 - Reflection 도메인 신설`. 신규 `src/domain/reflection.rs` (TurnSnapshot + compute_significance + ReflectionResult + 2 Placeholder + 8 단위 테스트). `src/domain/event.rs`에 `EventKind::DialogueReflected` + `EventPayload::DialogueReflected` variant + `EventPayload::DialogueEndRequested.reflection: Option<ReflectionResult>` 필드 (`#[serde(default)]` forward-compat). `Command::EndDialogue`에 동일 필드 + 5 callsite 갱신 (Director / DialogueOrchestrator / 3 통합 테스트 / RelationshipPolicy in-file 테스트 / dispatcher build_initial_event). RelationshipPolicy의 destructure는 `reflection: _` ignore (Stage 2에서 게이트 활성화). Director 경로는 spec §10.11 그대로 Phase 1.5 이연 (현재 `reflection: None` 명시). 회귀: **1076 passed / 0 failed / 7 ignored** (이전 1068 + 신규 8 = 1076). 단위 테스트 8개: empty 0 / peak 단독 / pad trajectory (3 turn windows 보정) / occ diversity 5 cap / beat binary 0.15 delta / clamp 1.0 / serde roundtrip 전체 / serde minimal default. **Stage 2 진입 가능**. |
| v0.8 | 2026-05-11 | **Stage 2 (Application) 완료** — commit `641bedb` `feat(application): Phase 1 Stage 2 - Reflection 인프라 + RelationshipPolicy 게이트`. **신규 3 파일**: `src/ports/reflection.rs` (ReflectionPort trait + ReflectionPrompt + ReflectionError, chat gated) / `src/adapter/reflection_via_chat.rs` (ConversationBackedReflectionPort, reflection_sid는 epoch_ms+counter — F8.2 uuid 우회) / `src/application/reflection_service.rs` (ReflectionRunner trait + ReflectionService<P> + DefaultReflectionPromptBuilder — compass만 prompt, taboo/life_question 제외 — F6 보정 / fallback_result 보수적 / 5 단위 테스트 Mock). **수정**: `RelationshipPolicy.handle_dialogue_end` 게이트 + 4 follow-up 순서 (DialogueReflected → RelationshipUpdated 조건부 → EmotionCleared → SceneEnded) + outer_loop_entry() pub(crate) helper + 5 신규 단위 테스트 (chitchat skip / significant 4-emit / legacy compat / declarative override / truth table). `dispatcher.MAX_EVENTS_PER_COMMAND` 21→22 (F3 (아) / §11.7). `DialogueOrchestrator`에 `reflection_service: Option<Arc<dyn ReflectionRunner>>` + `turn_buffers: HashMap` + `with_reflection()` 빌더 + `turn()` TurnSnapshot 누적 (reflection 부착 시만, leaky bucket 회피) + `end_session()` reflection 호출 + `current_emotion_pairs()` 헬퍼 + `run_reflection()` 내부 helper (NPC lookup scope 최소화). generic `<R, C>` 보존 — F2 #2 보정 그대로. **회귀 영향 catch**: `tests/memory_telling_test.rs::budget_exhaustion_test_20_listeners_succeeds_but_21_fails` → `..._21_listeners_succeeds_but_22_fails`로 갱신 (MAX_EVENTS 22 인상 영향). 회귀: **1086 passed / 0 failed / 7 ignored** (이전 1076 + 신규 10 = 1086). **OCP 검증**: src/application/에서 ConversationBackedReflectionPort import 0건 (주석만). **Stage 3 (외부 면 노출 — DTO + domain_sync + MCP + REST) 진입 가능**. |
| v0.9 | 2026-05-11 | **Stage 3 (외부 면 노출) 완료** — commit `f91ffe9` `feat(api): Phase 1 Stage 3 - 외부 면 노출 (AfterDialogueResponse.reflection + chitchat 호환)`. **수정 4 파일**: `src/application/dto/relationship.rs`에 `AfterDialogueResponse.reflection: Option<ReflectionResult>` 필드 (`#[serde(default)] + skip_serializing_if = "Option::is_none"` — forward-compat). `src/application/dialogue_orchestrator.rs::build_end_dialogue_from_v2` chitchat 호환 재작성 (DialogueReflected에서 reflection 회수 + RelationshipUpdated 미발행 시 SceneEnded npc/partner로 현재 relationship snapshot 조회 → before==after axes 보존). `src/bin/mind-studio/domain_sync.rs::dispatch_end_dialogue` Command::EndDialogue에 `reflection: None` 명시 (Stage 1 누락분 — Mind Studio 경로는 turn_buffers 미보유, spec §3.2 결정 (사) 그대로) + build_after_dialogue chitchat fallback. `src/bin/mind-studio/events.rs::StateEvent::DialogueReflected` variant 선언만 추가 (Phase 1.5 frontend 사전 배선, 현재 EventBus subscriber 미발행). **자동 흡수 (변경 0)**: MCP server (dialogue_end tool) / REST handlers — AfterDialogueResponse serde derive로 reflection 자동 직렬화. 회귀: **1086 passed / 0 failed / 7 ignored** (Stage 2와 동일, 회귀 0). `cargo build --features mind-studio,chat,embed --bin npc-mind-studio` ✅ 80s. **Stage 4 (Narrative validation 3 시나리오) 진입 가능**. |
| v0.10 | 2026-05-11 | **Stage 4 (Narrative validation) 완료** — commit `0078c5c` `test(phase1): Stage 4 narrative validation - 3 시나리오 + 통합 테스트`. **신규 시나리오 4 파일** (`data/scenarios/phase1-validation/`): chitchat-passerby.json (낮음, 4 turn) / daily-training.json (중간, 8 turn) / lin-chong-shanshenmiao.json (높음, 9 turn) + README.md (디자이너 검증 체크리스트). 3 시나리오 모두 `npc.inner_compass` 명시 — A-min 검증 포함. **신규 통합 테스트 3 파일**: phase1_chitchat_test.rs (3 follow-up + axes 보존 검증) / phase1_daily_training_test.rs (4 follow-up + axes 변화 발생 검증) / phase1_shanshenmiao_test.rs (큰 변화 + reasoning 박제 검증). Mock LLM (ReflectionResult를 직접 Command::EndDialogue payload에 주입) — 결정론 수 ms. 실제 LLM 검증은 디자이너(Bekay) Mind Studio 수동 (spec §6.4 결정). **테스트 import 노트**: `get_npc/save_emotion_state/get_relationship`은 `ports::{NpcWorld, EmotionStore}` sub-trait 직접 import 필요 (MindRepository super-trait만으로 method 미해결). 회귀: **1089 passed / 0 failed / 7 ignored** (Stage 3 1086 + 신규 narrative 3 = 1089). **Stage 5 (Bench — latency + significance 가중치 calibration) 진입 가능**. |
| v0.11 | 2026-05-11 | **Stage 5 (Bench) 완료** — commit `c7e1ac4` `test(phase1): Stage 5 Bench - latency + significance calibration`. 신규 `tests/phase1_bench_test.rs` (6 cases, criterion crate 도입 0). **결과**: (1) `compute_significance(10 turn) ×10000` per-call **8.36 µs** (target <1ms, 100x 마진). (2) `dispatch_v2(EndDialogue)` per-call (N=50): chitchat 24.17µs / significant 35.03µs / legacy 29.34µs. **chitchat vs legacy: 0.82x** (★ skip 효과 — 18% 더 빠름, RelationshipUpdated 미발행 cascade 감소). significant vs legacy: 1.19x (DialogueReflected 1개 추가 — 절대값 35µs 무시 가능, 실제 LLM ~수초 dominant). (3) **Calibration 3 밴드 모두 정확** — chitchat 0.000 (<0.3) / daily 0.461 (0.3~0.7) / shanshenmiao 0.980 (≥0.7). 가중치 0.40/0.30/0.15/0.15 + 임계값 0.3 → 디자이너 튜닝 불필요 (현 시점). 실제 LLM bench (~2~5초) + KV 캐시 효율은 spec §5.3 그대로 Mind Studio 수동. 회귀: **1095 passed / 0 failed / 7 ignored** (Stage 4 1089 + bench 6 = 1095). **Stage 6 (Migration archive) 진입 가능**. |
| v0.12 | 2026-05-11 | **★ Stage 6 (Migration archive) 완료 — Phase 1 종결**. 신규 `docs/changes/phase1-mind-architecture.md` (외부 사용자 API 변경 안내 — breaking 4종 + 신규 6종 + 환경 5건 + 성능 표). 신규 `docs/tasks/mind-architecture/phase1-checkpoint-report.md` (Stage 0~5 종합 — 한 문단 요약 + commit 표 + 13 결정 / 5 환경 이슈 / 후속 과제 / 게이트 통과 표). `00-roadmap.md` Phase 1 ✅ 완료 표기. `CLAUDE.md` 구현 현황 표에 Phase 1 행 추가 + External Document Index에 checkpoint report + API 변경 안내 링크. **Phase 2 (4축 + BondKind + Channel 1) 진입 가능**. |
