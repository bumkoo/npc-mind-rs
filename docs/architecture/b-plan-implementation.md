...
현재 사양은 `CLAUDE.md #핵심-진입점` 및 소스(`src/application/director/`,
`src/application/command/dispatcher.rs`)가 단일 진실 공급원입니다. §8의 "이행 로드맵"
체크리스트가 실제 커밋 기록에 가장 근접합니다.
...
| B1 | ✅ 완료 | 4 Policy + StimulusPolicy 신규 · `AppraiseRequested`/`StimulusApplyRequested` variant · `HandlerTestHarness` |
...
| B4 S1 | ✅ 완료 | 6 Command 전부 v2 지원 · ScenePolicy 신규 · 4 추가 *Requested variant · HandlerShared clear 시그널 |
...
| B5.2 (1/3) | ✅ 완료 | `DialogueOrchestrator` v2 `dispatch_v2` 마이그레이션 — 3개 dispatch 호출 전환 + DispatchV2Output→DTO 재구성 헬퍼 + emit_dialogue_turn deprecated projections() 호출 제거 + `#![allow(deprecated)]` 해제 |
...
| B5.2 (3/3) | ✅ 완료 | `AppState.shared_dispatcher` + `rebuild_repo_from_inner()` 도입. per-request snapshot·ephemeral dispatcher 제거. UI CRUD/scenario load가 공유 repo를 갱신하고 dispatch는 재사용. |
| B5.3 | ✅ 완료 | v1 모듈·타입 일괄 삭제 (Pipeline/Projection trait/EventAware/MindService/FormattedMindService/HandlerContext·Output/v1 Policy handle_*/AppStateRepository mut/DialogueTestService struct/shadow_v2). follow-up A: HandlerError variant 세분화. |
| B5.4 | 불필요 | B5.3에서 shadow_v2 함께 제거. |
...
- 각 Scene은 독립 LLM 세션 · 독립 DialogueOrchestrator · 독립 turn 흐름
...
**G3.** 외부 관찰자(MemoryProjector, StoryAgent, SSE, UI)가 단일 EventBus에서 모든 Scene의 이벤트를 수신하되, 필요 시 `aggregate_key()`로 Scene별 demultiplex 가능.
...
└─────┬─────┘    └─────┬─────┘    └─────┬─────┘
      ...
      ┌────────────────┼────────────────┬──────────────┐
      ▼                ▼                ▼              ▼
  MemoryProjector      StoryAgent       SSE bridge     (future)
...
| `Dispatcher` | **공유 `Arc<CommandDispatcher<R>>`** (B4 S4에서 확정) | Scene task들이 공통 handler 목록·EventStore·EventBus를 재사용 |
...
> **⚠️ 설계 스냅샷** — 실제 B4 S4 구현은 아래 설계와 차이가 있습니다:
> - `SceneTask` struct 대신 `pub(super) fn spawn_scene_task(scene_id, Arc<CommandDispatcher>, &Arc<dyn Spawner>) -> mpsc::Sender<Command>` 함수.
> - `tokio::spawn` 직접 호출 X → **`Spawner` trait 주입**(runtime-agnostic, `tokio/rt` 의존 없음).
> - `DialogueOrchestrator` 통합 미구현 (B5.2에서 DialogueOrchestrator가 v2 dispatch_v2 경로로 이행될 때 재설계).
> - v1 `dispatcher.dispatch(...)` 호출 대신 async `dispatch_v2(cmd).await`.
>
> 현재 구현: `src/application/director/scene_task.rs`, `src/application/director/spawner.rs`.

```rust
// src/application/director/scene_task.rs — B4 S4 최초 설계 (참고용)

pub struct SceneTask<R: MindRepository> {
    pub scene_id: SceneId,
    cmd_rx: mpsc::Receiver<Command>,
    dispatcher: Arc<CommandDispatcher<R>>,  // 공유 Dispatcher
    dialogue: Option<DialogueOrchestrator<R, RigChatAdapter>>,  // 이 Scene의 LLM 세션
}
...
impl<R: MindRepository + 'static> SceneTask<R> {
    pub fn spawn(
        ...
        dialogue_factory: impl FnOnce() -> DialogueOrchestrator<R, RigChatAdapter>,
    ) -> mpsc::Sender<Command> {
    ...
    async fn handle_command(&mut self, cmd: Command) -> Result<(), DispatchError> {
        match cmd {
            Command::DialogueTurn { utterance, .. } => {
                // LLM 호출이 필요한 커맨드는 DialogueOrchestrator 경유
                if let Some(dialogue) = &mut self.dialogue {
                    let _response = dialogue.turn(&self.scene_id.to_string(), utterance, None, None).await?;
                    // DialogueOrchestrator 내부가 Dispatcher로 이벤트 발행함
                }
                Ok(())
            }
            other => {
...
> **⚠️ 설계 스냅샷** — B4 S4 최종 구현과 다음이 다릅니다:
> - `new()` 시그니처: `Director::new(dispatcher: CommandDispatcher<R>, spawner: Arc<dyn Spawner>)`
>   — dispatcher를 소유(내부 `Arc::new`), bus/repo는 생성자 인자 아님(dispatcher에서 획득).
> - 메서드 이름: `send` 대신 **`dispatch_to`** (scene_id + target 커맨드 일치 검증 포함).
> - `start_scene` 시그니처: `(npc_id, partner_id, significance, focuses)` DTO 기반
>   (Scene 객체를 외부에서 만들지 않음; ScenePolicy가 내부에서 구성).
> - `end_scene` 시그니처: `(scene_id, significance: Option<f32>)` 2-arg.
> - `DirectorError`: `SceneNotActive` / `SceneAlreadyActive` / `SceneMismatch` /
>   **`SceneChannelClosed`** / `Dispatch` 5 variants (`SceneNotFound` 미채택).
> - TOCTOU race 방지를 위해 `start_scene`은 `write()` lock을 critical section 전체에 유지
>   (check + spawn + first send + insert가 원자적).
>
> 현재 구현: `src/application/director/mod.rs`.

```rust
// 아래는 B4 S4 최초 설계 (참고용) — 실제 구현은 위 노트 참조.
...
|---|---|---|---|
| `EmotionPolicy` | 10 | yes | `AppraiseRequested` |
| `StimulusPolicy` | 15 | yes (BeatTransitioned 발행 가능) | `StimulusApplyRequested` |
| `GuidePolicy` | 20 | no | `EmotionAppraised`, `StimulusApplied` |
| `RelationshipPolicy` | 30 | no | `SceneEnded`, `BeatTransitioned` |
...
| 구독자 | 활성 feature | 목적 |
|---|---|---|
| `MemoryProjector` | `embed` | 대화·관계 이벤트를 RAG에 인덱싱 |
...
impl<R: MindRepository> DispatcherBuilder<R> {
    ...
    pub fn for_production(repo: Arc<R>, event_store: Arc<dyn EventStore>) -> Self {
        Self::new(repo, event_store)
            .register(Arc::new(EmotionPolicy::default()))
            .register(Arc::new(StimulusPolicy::default()))
            .register(Arc::new(GuidePolicy::default()))
            .register(Arc::new(RelationshipPolicy::default()))
...
### Stage B1 — 기존 Policy가 EventHandler 추가 구현 ✅ 완료

**목표:** EmotionPolicy, StimulusPolicy(신규 분리), GuidePolicy, RelationshipPolicy가 새 트레이트 구현.

**작업:**
- [x] 각 Policy의 기존 로직을 v1 메서드로 유지, v2 `impl EventHandler`는 병렬 구현
- [x] `impl EventHandler for EmotionPolicy` / `StimulusPolicy` / `GuidePolicy` / `RelationshipPolicy`
- [x] 각 Policy에 대한 L1 단위 테스트 작성 (`HandlerTestHarness`)
- [x] 현재 `apply_stimulus`의 Beat 전환 로직을 `StimulusPolicy`로 분리 + BeatTransitioned follow_up 발행 검증
- [x] `EventPayload::AppraiseRequested` · `StimulusApplyRequested` variant 추가 (v2 커맨드 진입용)

**Acceptance:** ✅ 모든 Policy가 새 트레이트 구현 + L1 12 테스트 통과 + 기존 통합 테스트 무영향.
...
- [x] GuidePolicy/RelationshipPolicy 확장 (interest + shared→repo fallback)
- [x] `ScenePolicy` 신규 (priority::SCENE_START = 5, Scene 빌드 + 초기 focus appraise)
...
- [x] `StimulusPolicy` Beat 분기가 `ctx.repo.get_scene_by_id(&scene_id)` 사용 → multi-scene 오동작 수정
- [x] `RelationshipPolicy::handle_beat_transition`의 `ctx.repo.get_scene()` fallback 제거
...
**작업 (단계별 PR):**
- **B5.1 ✅ 완료**: `Pipeline`/`PipelineState`/`PipelineStage`, `Projection` trait/`ProjectionRegistry`, `CommandDispatcher::dispatch/execute_pipeline/register_projection/with_projections/projections()`, `handler::HandlerContext`/`HandlerOutput`, Policy v1 메서드 (`handle_appraise`/`handle_stimulus`/`handle_generate`/`handle_update`/`handle_end_dialogue`), `EventAwareMindService` 전체에 `#[deprecated(since = "0.2.0", note = "v0.3.0에서 제거 예정")]` 마킹. 내부 호출자 · 테스트 파일 상단에 `#![allow(deprecated)]` 추가. `cargo check` warning 0건.
- **B5.2 ✅ 완료**:
  - (1/3) DialogueOrchestrator v2 `dispatch_v2` 마이그레이션
...
- **B5.3 ✅ 완료**: `Pipeline`/`PipelineState`, v1 `Projection` trait/`ProjectionRegistry`, v1 `dispatch`/`execute_pipeline`, `EventAwareMindService`/`MindService`/`FormattedMindService`, v1 `handler` 모듈, v1 Policy `handle_*`, `AppStateRepository(mut)`, `DialogueTestService` struct, `shadow_v2` 플래그 전부 삭제. `emotion_snapshot` 헬퍼 → `EmotionState::snapshot()` 메서드, `MindServiceError` → `application::error` 모듈로 이관. v1 테스트 파일 8종(application/event/command/pipeline/locale/port_injection/repository/coverage_gap) + `dispatch_v2_test`의 v1 parallel 테스트 3종 삭제. Follow-up A: `HandlerError::Precondition` variant 세분화.
...
- DialogueOrchestrator의 LLM 세션 end_session
- (RelationshipPolicy가 EndDialogue 커맨드 처리로) 관계 갱신
- Scene의 final 이벤트 `SceneEnded` 발행

순서:
```rust
director.send(scene_id, Command::EndScene { significance }).await?;
// → RelationshipPolicy가 관계 갱신
// → SceneEnded 이벤트 발행
// → (MemoryProjector가 RAG 인덱싱)
director.end_scene(scene_id).await?;
// → mpsc::Sender drop → SceneTask 종료
// → DialogueOrchestrator drop → LLM 세션 종료
```
...
| DialogueOrchestrator.turn()이 `&mut self`인데 SceneTask 내부라 괜찮음 | - | - | SceneTask가 `self.dialogue: Option<DialogueOrchestrator>`로 소유. 동시 호출 불가능 |
...
- [`../memory/03-implementation-design.md`](../memory/03-implementation-design.md) — Memory System 확장 (Step A~D ✅ 완료 / Step E 선택 / Step F 향후). B5 이후 Phase 3 확장 경로로, CQRS 경계는 유지하면서 MemoryStore/MemoryRanker/Rumor/WorldOverlay/SceneConsolidation 서브시스템을 단계적으로 도입했다.

---

## 부록 A. EventHandler 구현 예시 — EmotionPolicy

```rust
// src/application/command/policies/emotion_policy.rs
...
impl EventHandler for EmotionPolicy {
    fn name(&self) -> &'static str { "EmotionPolicy" }
