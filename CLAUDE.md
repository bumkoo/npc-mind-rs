...
cargo test --test directive_test      # ActingDirective 의사결정 트리 전 분기 검증 (32개)
...
cargo test --test locale_test         # 언어 설정 + 플러거블 포맷터 (20개)
...
tests/
  ...
  directive_test.rs               # ActingDirective Tone/Attitude/Behavior/Restriction 전 분기 (32건)
  coverage_gap_test.rs            # valence, merge 경계값, PAD 좌표, 수식 정밀 검증 (23건)
  locale_test.rs                  # 언어 설정 + 플러거블 포맷터 (20건)
  port_injection_test.rs          # 포트 주입 + Scene/Beat 통합 (14건)
docs/
...
apply_stimulus 호출
  ...
  → 4. StimulusResult.beat_changed = true, active_focus_id 업데이트
```

### Scene 초기화 공통 로직
`start_scene()`과 `load_scene_focuses()`는 내부적으로 공통 헬퍼를 사용합니다:
- `save_scene_state()`: Focus 목록 + NPC/파트너 ID 저장
- `appraise_initial_focus()`: Initial Focus 찾기 → appraise → 감정 저장
