...
- **안전 한계**: `MAX_CASCADE_DEPTH = 4`, `MAX_EVENTS_PER_COMMAND = 22` (Phase 1에서 21 → 22로 인상 — `EndDialogue`의 `DialogueReflected` 추가에 따른 worst-case 증가. `TellInformation`(청자 N개) 등 기반 실측 상한. 정확한 산정·테스트는 `application/command/dispatcher.rs` 참조)
...
| **Mind Architecture Phase 1.5 (Mind Studio Reflection 통합)** | ✅ 완료 (2026-05-12, `5cf8fb5` → `9b35e99`) | Phase 1의 reflection 게이트를 Mind Studio의 ad-hoc 경로(`state.chat + domain_sync`)에 mirror — DialogueOrchestrator를 사용하지 않으므로 mirror 패턴 채택. **Backend**: `AppState.reflection_service: Option<Arc<dyn ReflectionRunner>>` + `with_reflection()` + `StateInner.turn_buffers: HashMap<String, Vec<TurnSnapshot>>` (chat-gated, `#[serde(skip)]`) + `StudioService::run_reflection_for_session` + `perform_after_dialogue(state, req, session_id)` 시그니처 확장 + `domain_sync::dispatch_end_dialogue(state, inner, req, reflection)` 시그니처 확장. 부팅 시 *별도* `RigChatAdapter` 인스턴스 → `ConversationBackedReflectionPort` → `DefaultReflectionPromptBuilder` → `ReflectionService` 합성 자동 부착 (dialogue 세션과 KV 캐시 분리). **Frontend**: `ReflectionView.tsx` 신규 (chitchat/significant 라벨 + significance band + summary + reasoning + axes Δ) + ResultPanel '반추' 탭 + `useResultStore.lastAfterDialogue` + `handleEndChat` toast band + `useStateSync.dialogue_reflected` 핸들러. 4 통합 테스트 (Mock ReflectionRunner) + 1 vitest. 디자이너가 UI '반추' 탭에서 chitchat/significant 결과 직접 확인 가능. **알려진 trade-off**: DialogueOrchestrator와 Mind Studio가 *동일 reflection 로직을 두 군데*에 보유 → drift 위험 (mitigation은 mirror 주석 + 동일 시그니처 + 대칭 테스트). Mind Studio 외부 노출 또는 drift 누적 시 cutover 검토. |
| **Mind Architecture Phase 1.6 (EventBus → SSE Bridge)** | ✅ 완료 (2026-05-12, `fb22400`) | Mind Studio의 도메인 관련 SSE 발행을 EventBus 구독으로 일원화. `src/bin/mind-studio/event_bridge.rs` 신설 (~250 LoC, `MemoryProjector` 패턴 mirror — `subscribe_with_lag` + Lagged 시 `EventStore::get_events_after_id` replay). `map_event(&DomainEvent) -> Vec<StateEvent>` 결정론 매핑 9개: `EmotionAppraised`→`Appraised`, `StimulusApplied`→`StimulusApplied`, `GuideGenerated`→`GuideGenerated`, `SceneStarted`→`SceneStarted`, `SceneEnded`→`AfterDialogue`, `DialogueTurnCompleted(speaker=assistant)`→`ChatTurnCompleted`, `DialogueReflected`→`DialogueReflected`, `MemoryEntry{Created,Superseded,Consolidated}`→동명, `Rumor{Seeded,Spread}`→동명. `main.rs`에서 `tokio::spawn(bridge.run(...))` 부팅 시 1회. **manual `state.emit()` 11곳 제거** (studio_service 5 + mcp_server 2 + handlers/scenario 2 + handlers/rumor 2). **UI-only emit 유지**: `HistoryChanged`/`SituationChanged`/`TestReportChanged`/`SceneInfoChanged`/`ScenarioLoaded`/`ScenarioSaved`/`ResultLoaded`/`NpcChanged`/`RelationshipChanged`/`ObjectChanged`/`ChatStarted`/`ChatEnded`/`MemoryCreated/Superseded` (Memory 이벤트 EventBus 미발행, Step F 대기). **보너스**: `/api/v2/scenes/*` Director 경로 SSE silent bug 부수 fix (단 `director_v2`는 *별도 dispatcher*라 본 bridge 범위 외 — shared_dispatcher 통합은 별도 작업). `StateEvent`에 `PartialEq`/`Eq` derive (매핑 비교 검증용). 7 단위 + 1 통합 테스트 신규. |
| **Mind Architecture Phase 2 (Relationship 4축 도메인 안정화)** | ✅ 완료 (2026-05-16, Stage 0~6 종결) | `relationships.md` v0.7 — Relationship 3축(closeness/trust/power × ±1.0) → **4축 (trust/affinity/respect/wariness × ±100)** + 4 보조축 (BondKind 11 / BondStatus 5 / Partnership 4 / type 자유 텍스트) 도입. Stage 1 도메인 본체 (`domain/relationship/{mod,axis,bond,partnership}.rs`) + Stage 2 OCC→4축 매핑 (`mapping.rs` 48셀 base_delta + HEXACO 6 보정룰 + `update_axes_from_emotion` 단일 진입점) + Stage 3 wire payload 6→8 필드 + ÷100 layer 4겹 제거 (B-D-A ±100 raw) + Stage 4 시나리오 4파일 v0.7 영구 변환 + v0.6 code 0건화 + Stage 5 narrative S1~S3 정량 박제 + D3 3밴드 exact 보존 (0.000/0.461/0.980) + Stage 6 bench 회귀 0 + Phase 2.3 인계. 회귀 0 (843P/0F/2I/65 묶음, `--lib --tests --bins`). D2 latency 임계값 50% 마진 (chitchat 15.70/29 / significant 26.68/42 / legacy 20.89/35.2 µs). B-D 본체 12 결정 + Stage 3 추가 4 결정 + Stage 5·6 측정 정본 6+5 결정 박제. B-D7/B-D11/axis_modulation는 Phase 2.5 위임. 디자이너 잔여: intensity 0.4 / S1~S3 narrative 타당성 / S4 정성 (Phase 2.3 KICKOFF §5). 부채 (Phase 2 시점 인계 항목, **Phase 2.3에서 처리 완료 — 2026-05-17 확인② 실측 확정**): ÷100 잔존 → §A 값 동치 제거(weight 1/100), closeness/power 12파일/69매치 → affinity rename + power_level 폐기, state.rs 커스텀 Deserialize → Stage 4(`81777e0`)에서 이미 제거였음이 Stage 0 사실조사로 정정 확정(KICKOFF §1-E 오독, FROZEN spec §0.5), B-D9 result.json dump → Phase 2.5 이관. 상세: [`docs/tasks/mind-architecture/phase2-checkpoint-report.md`](docs/tasks/mind-architecture/phase2-checkpoint-report.md), Phase 2.3 정본 [`task-rel-phase2.3-appraise-tuning.md`](docs/tasks/mind-architecture/task-rel-phase2.3-appraise-tuning.md). |
...
- **Phase 1 checkpoint report**: [`docs/tasks/mind-architecture/phase1-checkpoint-report.md`](docs/tasks/mind-architecture/phase1-checkpoint-report.md) (v0.2) — Phase 1 본체 (Stage 0~5) + Phase 1.5 (Mind Studio 통합) + Phase 1.6 (EventBus→SSE bridge) 종합. §7 *"Phase 2 진입 전 알아야 할 현재 구현 상태"* 신설 — Phase 2 spec 작성자 필독.
...
- **Phase 1/1.5/1.6 API 변경 안내**: [`docs/changes/phase1-mind-architecture.md`](docs/changes/phase1-mind-architecture.md) — 외부 사용자용 breaking change 모음 (`Command::EndDialogue.reflection` 필드 / `Npc.inner_compass` / `MAX_EVENTS_PER_COMMAND` 22 / `AppState.reflection_service` / `StateInner.turn_buffers` / `StudioService::perform_after_dialogue(... , session_id)` 시그니처 / `domain_sync::dispatch_end_dialogue(..., reflection)` 시그니처 / `event_bridge::EventBridge` 신규 / manual `state.emit()` 11곳 제거 / 환경 이슈 5건).
- **Phase 2 spec (정본)**: [`docs/tasks/mind-architecture/task-rel-phase2-domain-migration.md`](docs/tasks/mind-architecture/task-rel-phase2-domain-migration.md) **v1.0 FROZEN** — Relationship 4축 도메인 마이그레이션. §7 Stage 1~6 모두 종결. B-D 12 결정 + Stage 3 추가 4 결정 박제.
- **Phase 2 checkpoint report**: [`docs/tasks/mind-architecture/phase2-checkpoint-report.md`](docs/tasks/mind-architecture/phase2-checkpoint-report.md) — Stage 0~6 종합. D baseline 최종 대조표 + B-D 결정 최종 상태 + 4축 안정화 결과 + 부채 인계 카탈로그 + 종결 게이트 통과 확인.
- **Phase 2 Stage 회고들**: [`phase2-stage1-domain.md`](docs/tasks/mind-architecture/phase2-stage1-domain.md) / [`phase2-stage2-mapping.md`](docs/tasks/mind-architecture/phase2-stage2-mapping.md) / [`phase2-stage3-domain-wire-frontend.md`](docs/tasks/mind-architecture/phase2-stage3-domain-wire-frontend.md) / [`phase2-stage4-migration.md`](docs/tasks/mind-architecture/phase2-stage4-migration.md) / [`phase2-stage5-narrative.md`](docs/tasks/mind-architecture/phase2-stage5-narrative.md) / [`phase2-stage6-bench-handoff.md`](docs/tasks/mind-architecture/phase2-stage6-bench-handoff.md)
- **Phase 2.3 KICKOFF**: [`docs/tasks/mind-architecture/PHASE2.3-KICKOFF.md`](docs/tasks/mind-architecture/PHASE2.3-KICKOFF.md) v1.2 — Appraise tuning + ±100 native 잔존 청소 + Stage 4·5·6 인계 5항 + 역대조 게이트 + closeness/power 12 파일/69 매치 재카탈로그 플래그.
- **Phase 2.3 spec (정본)**: [`docs/tasks/mind-architecture/task-rel-phase2.3-appraise-tuning.md`](docs/tasks/mind-architecture/task-rel-phase2.3-appraise-tuning.md) — 🟢 **FROZEN → 종결** (2026-05-17, 확인② 실측 검증). P-D-1~9 / C-1~6 / 3 Stage(2.3-A 값동치 ±100 / 2.3-B comment-drift sweep / 2.3-C narrative S1~S4). 회귀 0 (`cargo test --lib` 554P/0F). §0.5에 KICKOFF v1.2 §1-C/§1-E 정정 박제 (§C max≠sum, §E Stage 4 이미 제거). 검증 보고: [`phase2.3-checkpoint2-verification.html`](docs/tasks/mind-architecture/phase2.3-checkpoint2-verification.html).
