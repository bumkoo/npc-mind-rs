...
- **기존 `MemoryAgent` / `SqliteMemoryStore`의 확장 지점**을 도출한다 (구현은 3차).
...
│  - MemoryAgent (확장), RetentionService,               │
...
| `RumorDistorted` | 왜곡 버전 추가 | rumor_id, distortion_id, parent_distortion, content | (향후) 서사 에이전트 |
...
|---|---|---|---|
| `Command::TellInformation { speaker, listeners, claim, stated_confidence }` | **Mind** (정보 전달은 대사/서사 사건) | `InformationToldRequested` | `InformationAgent`(Mind) → **청자별로 `InformationTold { listener }` N개 follow-up 발행** (B5). Memory는 §6.2 구독으로 처리. 1차 §3.4.1 **판정 경로 (a)** 명시 발행. |
| `Command::ApplyWorldEvent { topic?, updated_fact, significance }` | **Mind** (세계 사건 자체가 Mind 도메인 이벤트) | `WorldEventRequested` | `WorldOverlayAgent`(Mind) → `WorldEventOccurred` follow-up. Memory는 §6.2 `WorldOverlayPolicy`로 오버레이+Supersede 처리. 1차 §3.4.1 **판정 경로 (a)** 명시 발행. |
| `Command::SeedRumor { topic?, content, reach, origin }` | **Memory** | `RumorSeedRequested` | `RumorAgent` → `RumorSeeded` |
| `Command::SpreadRumor { rumor_id, extra_recipients? }` | **Memory** | `RumorSpreadRequested` | `RumorAgent` → `RumorSpread` follow-up → MemoryEntry 생성 (트랜잭션 일관성, I-RU-5) |
...
> **`InformationTold` 청자당 1 이벤트 (B5)**: 한 커맨드 `TellInformation`이 N명의 청자를 가지면 InformationAgent는 N개의 `InformationTold` 이벤트를 발행한다 (각각 `listener: NpcId` 단일). 이렇게 하면:
...
> **hook 인터페이스 (3차 구체화)**: `RelationshipUpdated` 이벤트 payload에 `cause: RelationshipChangeCause` enum을 추가한다. RelationshipAgent는 변경 발행 시점에 원인을 이벤트에 각인해야 한다 (A8 보장). 이 enum의 구체 variant는 3차에서 확정.
...
- 신규 핸들러: `InformationAgent`, `RumorAgent`, `WorldOverlayAgent`, `SceneConsolidationHandler` (Transactional).
- 기존 `MemoryAgent`는 **Inline 핸들러 또는 EventBus 구독자**로 역할을 명확화:
...
|---|---|---|
| `WorldOverlayAgent` | **25** | Guide 직후, Relationship 이전. 세계 오버레이가 장면 프롬프트 guide에는 반영되지 않되, 관계 갱신에는 반영될 수 있도록 RelationshipAgent보다 앞에 둔다. |
| `InformationAgent` | **35** | Relationship 갱신 직후. 정보 전달로 관계가 변한 경우 `RelationshipUpdated.cause = InformationTold`가 이미 기록된 상태에서 `InformationTold { listener }`를 발행. |
| `RumorAgent` | **40** | 정보 전달 이후, 마지막 확산 처리. Rumor 확산은 장면 외부 전파라 장면 흐름 후순위. |
| `SceneConsolidationHandler` | **45** | Scene 종료 지점에 마지막으로 실행. 통합은 다른 모든 관련 Memory 생성이 커밋된 뒤에야 입력이 완결된다. |

priority 상수의 최종 이름·값은 3차에서 확정. 현재 간격(5 단위)은 장래 Agent 삽입 여지를 유지하기 위함.
...
- 쿼리 API는 `DialogueAgent`·`Director`·Mind Studio가 사용.
...
- 기존 `MemoryAgent` / `SqliteMemoryStore` / `MemoryStore` trait 마이그레이션 전략
