...
src/
  ...
  bin/mind-studio/  Axum REST API + SSE MCP 서버 + SSE 실시간 동기화 + static 파일 서빙
                  ...
                  - domain_sync.rs  StateInner ↔ InMemoryRepository per-request snapshot + 5 dispatch helper (appraise/stimulus/end_dialogue/generate_guide/start_scene)
...
- B5.2 (1/3)에서 v1 `dispatch()` → v2 `dispatch_v2().await`로 내부 전환 완료. 외부 API 시그니처 불변.
...
### 도메인 동기화 (`domain_sync.rs`)

각 REST/MCP 핸들러는 다음 패턴을 `domain_sync`의 dispatch helper를 통해 캡슐화:
1. `state.inner.write().await` 획득
2. `snapshot_to_repo(&inner)` — StateInner (UI 스키마) → `InMemoryRepository` 복제
3. `build_ephemeral_dispatcher(repo)` — `with_default_handlers()` 적용된 1회용 dispatcher
4. `dispatcher.dispatch_v2(cmd).await` — EventHandler 체인 실행
5. `HandlerShared` + `output.events` → UI DTO 재구성 (`AppraiseResult`/`StimulusResult`/...)
6. `sync_from_repo(&repo, &mut inner)` — 갱신된 관계/감정/Scene을 UI 레이어로 역반영
...
- `snapshot_to_repo`, `sync_from_repo`, `build_ephemeral_dispatcher`

**성능**: request당 핸들러 8개 Arc 재생성 + state HashMap 복사. 일반 UI 부하(분당 수십 request)에 무시 가능. B5.2 (3/3) AppState 통합 시 해소 예정.
