...
- **Libraries:** `serde`/`serde_json`, `thiserror`, `tokio/sync`+`tokio-stream`+`futures` (EventBus 내부 구현), `axum`(WebUI), `tracing`, `ort`(ONNX 임베딩), `rig-core`(LLM Agent 대화), `rusqlite`+`sqlite-vec`(RAG 저장소 [embed] — FTS5 + vec0 벡터 인덱스), `regex` (optional, `listener_perspective` feature — 한국어 정규식 프리필터)
...
src/
  ...
  domain/         순수 도메인 로직
                  ...
                  - listener_perspective [feature] (Phase 7 — 화자 PAD → 청자 PAD 변환: prefilter + sign/magnitude k-NN + Converter trait, 88% baseline)
...
- `::new(repo, event_store, event_bus)` / `.with_projections(registry)` / `.register_projection(p)`
- `dispatch(cmd)` — 직접 라우팅 / `execute_pipeline(pipeline, cmd)` — Pipeline 순차 실행
- `event_store()`/`event_bus()`/`projections()`/`repository()` 훅으로 내부 의존성 노출 (DialogueAgent 등 상위 래퍼가 동일 발행 경로를 재사용)
...
│  Appraise·ApplyStimulus·EndDialogue 커맨드를 dispatch하고   │
│  DialogueTurnCompleted(user/assistant)를 직접 발행          │
...
| Phase 4 | ✅ 완료 | DialogueAgent — CommandDispatcher + ConversationPort 통합 오케스트레이터. `DialogueTurnCompleted` 이벤트를 EventBus로 발행하여 MemoryAgent가 자동 인덱싱. chat feature |
...
- **Listener-perspective 변환** (Phase 7): [`docs/emotion/sign-classifier-design.md`](docs/emotion/sign-classifier-design.md) (부호/강도 분류기 설계 + §3.7 Register 전략) + [`docs/emotion/phase7-converter-integration.md`](docs/emotion/phase7-converter-integration.md) (프로덕션 통합, Step 1-3 완료 88%)
