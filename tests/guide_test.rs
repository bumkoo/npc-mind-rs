...
use npc_mind::ports::{Appraiser, GuideFormatter};
...
fn 무백_성격_스냅샷_정직_관용() {
    ...
    assert!(
        snapshot.traits.contains(&PersonalityTrait::HonestAndModest),
        "무백의 성격에 '정직겸손' 포함: {:?}",
        snapshot.traits
    );
    assert!(
        snapshot
            .traits
            .contains(&PersonalityTrait::TolerantAndGentle),
        "무백의 성격에 '관용온화' 포함: {:?}",
        snapshot.traits
    );
    assert!(
        snapshot
            .speech_styles
            .contains(&SpeechStyle::FrankAndUnadorned)
            || snapshot
                .speech_styles
                .contains(&SpeechStyle::SoftAndConsiderate),
        "무백의 말투에 솔직/부드러운: {:?}",
        snapshot.speech_styles
    );
}
...
fn 교룡_성격_스냅샷_교활_비판적() {
    ...
    assert!(
        snapshot
            .traits
            .contains(&PersonalityTrait::CunningAndAmbitious),
        "교룡의 성격에 '교활야심' 포함: {:?}",
        snapshot.traits
    );
    assert!(
        snapshot
            .traits
            .contains(&PersonalityTrait::GrudgingAndCritical),
        "교룡의 성격에 '비판적' 포함: {:?}",
        snapshot.traits
    );
    assert!(
        snapshot
            .traits
            .contains(&PersonalityTrait::CuriousAndCreative),
        "교룡의 성격에 '호기심' 포함: {:?}",
        snapshot.traits
    );
}
...
fn 배신_무백_가이드_절제된_분노() {
    ...
    assert!(
        prompt.contains("억누") || prompt.contains("절제") || prompt.contains("차가운"),
        "무백 가이드에 절제 관련 표현: {}",
        prompt
    );
    assert!(
        prompt.contains("호의적") || prompt.contains("금지"),
        "무백 가이드에 금지 사항 포함: {}",
        prompt
    );
...
fn 배신_교룡_가이드_폭발적_분노() {
    ...
    assert!(
        prompt.contains("거칠") || prompt.contains("공격"),
        "교룡 가이드에 공격적 표현: {}",
        prompt
    );
    assert!(
        prompt.contains("적대") || prompt.contains("공격적인 태도"),
        "교룡 가이드에 적대적 태도: {}",
        prompt
    );
...
fn 관계_없으면_json에_관계_없음() {
    ...
    assert!(
        parsed.get("relationship").is_none(),
        "관계 없으면 JSON에서 생략"
    );
}
