...
2.  **`to_domain(&self, repo, ...)`**: DTO에 구현된 변환 메서드.
    -   `NpcWorld` 포트를 사용하여 필요한 관계(Relationship)나 객체 정보(Object Description)를 조회한다. 관계는 `Relationship::modifiers()`를 통해 `RelationshipModifiers` VO로 변환된다.
...
-   **Fortune-of-others**: `desirability_for_other`가 존재할 경우, 대상에 대한 관계 modifier(`RelationshipModifiers`)를 고려하여 `HappyFor`, `Pity`, `Resentment`, `Gloating` 중 하나를 생성한다.
...
pub struct ActionFocus {
    ...
    pub modifiers: Option<RelationshipModifiers>,    // 제3자 행동인 경우 제공
}
```
...
-   **관계 보정**: 타인 행동 평가 시 `trust_modifier`와 `intensity_multiplier`가 강도에 반영된다.
...
// 3. 감정 평가 실행
let state = AppraisalEngine.appraise(&personality, &situation, &relationship.modifiers());
