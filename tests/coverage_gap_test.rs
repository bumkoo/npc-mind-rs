...
use npc_mind::domain::pad::{emotion_to_pad, pad_dot, Pad};
...
use common::{make_무백, score as s, TestContext};
...
fn overall_valence_빈_상태는_0() {
    ...
    assert_eq!(state.overall_valence(), 0.0,
        "감정 없으면 valence=0.0");
}
...
fn merge_동일_강도면_새것이_유지() {
    ...
    assert_eq!(merged.context_of(EmotionType::Anger).unwrap(), "새 원인",
...
fn merge_threshold_0이면_모든_이전_감정_유지() {
    ...
    assert!(merged.intensity_of(EmotionType::Fear) > 0.0,
...
fn merge_양쪽_다_여러_감정_있으면_각각_처리() {
    ...
    assert!((merged.intensity_of(EmotionType::Anger) - 0.8).abs() < 0.001,
        "Anger: prev(0.8) > new(0.4) → 0.8 유지");
    assert!((merged.intensity_of(EmotionType::Fear) - 0.6).abs() < 0.001,
        "Fear: new(0.6) > prev(0.3) → 0.6 유지");
    assert_eq!(merged.intensity_of(EmotionType::Shame), 0.0,
...
fn 역방향_관계_조회로_after_dialogue_성공() {
    ...
    assert!(result.is_ok(),
        "역방향 관계 조회로 after_dialogue 성공해야 함: {:?}", result.err());
}
...
fn emotion_to_pad_22개_전수_검증() {
    ...
    for (etype, exp_p, exp_a, exp_d) in expected {
        ...
        assert!((pad.pleasure - exp_p).abs() < 0.001,
            "{:?} P: got {}, expected {}", etype, pad.pleasure, exp_p);
        assert!((pad.arousal - exp_a).abs() < 0.001,
            "{:?} A: got {}, expected {}", etype, pad.arousal, exp_a);
        assert!((pad.dominance - exp_d).abs() < 0.001,
            "{:?} D: got {}, expected {}", etype, pad.dominance, exp_d);
    }
}
...
fn emotion_to_pad_긍정_감정은_p_양수() {
    let positive = [
        EmotionType::Joy, EmotionType::HappyFor, EmotionType::Hope,
        EmotionType::Satisfaction, EmotionType::Relief,
    ...
    for etype in positive {
        ...
        assert!(pad.pleasure > 0.0,
            "{:?}는 긍정 감정이므로 P > 0 이어야 함 (got {})", etype, pad.pleasure);
    }
}
...
fn emotion_to_pad_부정_감정은_p_음수() {
    let negative = [
        EmotionType::Distress, EmotionType::Pity, EmotionType::Resentment,
        EmotionType::Fear, EmotionType::Disappointment, EmotionType::FearsConfirmed,
    ...
    for etype in negative {
        let pad = emotion_to_pad(etype);
        assert!(pad.pleasure < 0.0,
            "{:?}는 부정 감정이므로 P < 0 이어야 함 (got {})", etype, pad.pleasure);
    }
}
...
fn format_prompt_빈_감정_상태() {
    ...
    assert!(!prompt.contains("[상대와의 관계]"), "관계 없으면 관계 섹션 없음");
}
...
fn format_prompt_관계_포함_빈_감정() {
    ...
    let brother = RelationshipBuilder::new("mu_baek", "gyo_ryong")
        .closeness(s(0.8)).trust(s(0.7)).build();
    ...
    assert!(prompt.contains("[상대와의 관계]"), "관계 있으면 관계 섹션 존재");
}
...
fn trust_음수_praiseworthiness면_하락() {
    ...
    assert!(updated.trust().value() < 0.5,
        "음수 PW → trust 하락: {}", updated.trust().value());

    // delta = -0.8 * 0.1 * 1.0 = -0.08
    let expected = 0.5 - 0.08;
    assert!((updated.trust().value() - expected).abs() < 0.001,
        "trust: got {}, expected {}", updated.trust().value(), expected);
}
...
fn closeness_음수_valence면_하락() {
    ...
    assert!(updated.closeness().value() < 0.3,
        "음수 valence → closeness 하락: {}", updated.closeness().value());

    // delta = -0.6 * 0.05 * 1.0 = -0.03
    let expected = 0.3 - 0.03;
    assert!((updated.closeness().value() - expected).abs() < 0.001,
        "closeness: got {}, expected {}", updated.closeness().value(), expected);
}
...
fn trust_경계값_클램핑() {
    ...
    assert!(updated.trust().value() <= 1.0,
        "trust 상한 클램핑: {}", updated.trust().value());
}
...
fn trust_하한_클램핑() {
    ...
    let updated = rel.with_updated_trust(-1.0, 1.0);
    assert!(updated.trust().value() >= -1.0,
        "trust 하한 클램핑: {}", updated.trust().value());
}
...
fn after_dialogue_praiseworthiness_none이면_trust_미갱신() {
    ...
    assert!((updated.trust().value() - 0.5).abs() < 0.001,
        "PW=None → trust 미갱신: {}", updated.trust().value());
    assert!(updated.closeness().value() > 0.0,
        "closeness는 항상 갱신: {}", updated.closeness().value());
}
...
fn after_dialogue_power는_변경_없음() {
    ...
    assert!((updated.power().value() - 0.7).abs() < 0.001,
        "power는 after_dialogue에서 변경 안 됨: {}", updated.power().value());
}
