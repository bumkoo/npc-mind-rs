...
- `.with_default_handlers()` — SceneAgent/EmotionAgent/StimulusAgent/GuideAgent/RelationshipAgent/InformationAgent/WorldOverlayAgent + 3 Projection wrapper 자동 등록
- `.with_memory(store)` — Memory 계열 Inline 핸들러 4종 부착 (TellingIngestionHandler/WorldOverlayHandler/RelationshipMemoryHandler/SceneConsolidationHandler)
- `.with_rumor(memory_store, rumor_store)` — RumorAgent (Transactional) + RumorDistributionHandler (Inline)
...
│          RelationshipMemoryHandler (D, pri 50) +             │
...
| `ApplyWorldEventRequested` | `Command::ApplyWorldEvent` | WorldOverlayAgent (WorldEventOccurred follow-up) — Step D |
...
| **Memory Step D** | ✅ 완료 | `Command::ApplyWorldEvent` + `ApplyWorldEventRequest` DTO + `WorldOverlayAgent` (Transactional, priority `WORLD_OVERLAY=25`) + `WorldOverlayHandler` (Inline, priority `WORLD_OVERLAY_INGESTION=45`) — Canonical `MemoryEntry(scope=World, provenance=Seeded)` 생성 + 같은 topic 기존 엔트리 supersede. `SceneConsolidationHandler` (Inline, priority `SCENE_CONSOLIDATION=60`) — `SceneEnded` 구독 → NpcAllowed 필터로 양쪽 NPC의 Layer A `DialogueTurn/BeatTransition` 엔트리 수집 → `SceneSummary` Layer B 엔트리 생성 + `mark_consolidated`. `RelationshipMemoryHandler` (Inline, priority `RELATIONSHIP_MEMORY=50`) — `RelationshipUpdated.cause` 5 variant별 source/topic/content 분기 (`MEMORY_RELATIONSHIP_DELTA_THRESHOLD=0.05` 미만 skip). `RelationshipAgent.BeatTransitioned` 경로에서 cause=`SceneInteraction { scene_id }` 설정. `with_default_handlers()` transactional 7종(+WorldOverlayAgent), `with_memory(store)` Inline 4종 (+WorldOverlay/SceneConsolidation/RelationshipMemory). 15개 통합 테스트 (consolidation 3 + world overlay 6 + relationship cause 6) + 14개 단위 테스트. 범위 외: LLM 기반 Consolidator / witness 개별 MemoryEntry / `TopicLatestProjection` 독립 struct는 후속 Phase. |
