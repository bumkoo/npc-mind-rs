...
**설계 결정: 대사 PAD = 화자의 감정 톤 (+ 청자 변환 알고리즘 필요)**

`analyze-utterance`는 대사 텍스트 자체의 감정 톤을 측정한다. 대사를 듣는 NPC의 감정이 아니다.

예: 헉의 사과 "미안해. 내가 잘못했어." → P=-0.339 (화자의 후회/자책 톤)
→ 짐의 관점에서는 P+(화해 수용)이어야 하지만, 분석기는 화자 톤을 측정하므로 P-가 나온다.

**실제 게임에서는 수동 보정 불가** → NPC에게 stimulus 적용 전 자동 변환 알고리즘 필요.

검토할 접근:
1. **화행(speech act) 분류기**: 대사 패턴으로 사과/위협/위로/요청 분류 → 화행별 PAD 변환 규칙
2. **관계 기반 변환**: 신뢰도/친밀도에 따라 같은 사과도 청자에게 다른 P값으로 적용
3. **LLM 기반**: 대사+관계 맥락을 LLM에 넘겨서 "듣는 사람이 느낄 PAD" 추론

**임시 수동 보정 참고 패턴** (알고리즘 구현 전 Mind Studio에서 테스트 시):

| 화자 대사 패턴 | 분석 결과 (화자 톤) | 청자에게 필요한 보정 |
|--------------|-------------------|-------------------|
| 사과/자책 | P-, A 약 | P를 +로 조정 (사과 수용) |
| 위로/격려 | P+, A- | 보정 불필요 (방향 일치) |
| 도발/위협 | P-, A+, D+ | 보정 불필요 (방향 일치) |
| 태연한 거짓말 | P+, A- | P를 -로 조정 (속임 인지 시) |
| 냉정한 거절 | P-, A- | A를 +로 조정 (충격/각성) |

수동 보정 시 WebUI 슬라이더에서 `analyze-utterance` 결과를 확인한 뒤, 청자 NPC의 관점에 맞게 조정한다.
...
| **청자 관점 PAD 변환** | analyze-utterance가 화자 톤만 측정 → 사과/위로가 P-로 나옴 | 화행 분류 → 청자 관점 PAD 자동 변환 알고리즘 | 높음 |
| **stimulus 감쇄 속도** | 강한 감정(Anger 0.8+)이 P+ stimulus에 느리게 반응 | MIN_INERTIA 하향(0.30→0.20~0.25) + IMPACT_RATE 소폭 상향(0.5→0.6) 검토 | 높음 |
| **Beat trigger 설계 가이드** | trigger 임계값이 시나리오마다 수동 조정 필요 | 감정 초기 강도 대비 비율 기반 trigger 또는 가이드라인 문서화 | 중간 |
...
### Phase 3: PAD 앵커 최적화 (진행 중)
**목표**: 대사 텍스트에서 정확한 감정 톤 추출
**장면**: 무협 도메인 대사 (청강만리, 와호장룡 스타일)
**초점**: 한국어/무협 어투 앵커 확장, before/after 비교

**진행 이력**:
- [2026-04] A+ 앵커에 감정적 고조(비전투) 표현 4개 추가 (10→14개)
  - 울음/통곡, 대인 분노 폭발, 격앙된 호소, 감격 환희
  - Ch.15 Trash Speech A값: 0.077→0.215 (+179%)
  - 벤치마크 A축: 16/19→15/19 (비탄 경계값 1건 탈락, 트레이드오프 허용)
  - 벤치마크 P축: 17/18→18/18 (간접 개선), D축: 13/17→14/17 (간접 개선)
- [2026-04] 설계 결정: 대사 PAD = 화자 감정 톤. 청자 관점은 수동 보정 (사과/위로 등)
- [2026-04] **설계 과제 발견**: 수동 PAD 보정은 실제 게임에서 불가능 → 청자 관점 PAD 자동 변환 알고리즘 필요
  - 접근 후보: 화행(speech act) 분류기, 관계 기반 변환, LLM 기반 추론
- [2026-04] session_003에서 Beat 전환 병목 확인: trigger 조건 엄격 + Distress 감쇄 느림
  - TODO: trigger 임계값 완화 (Anger<0.7, Distress<0.6)
  - TODO: STIMULUS_MIN_INERTIA 하향 검토 (0.30→0.20~0.25)
  - TODO: STIMULUS_IMPACT_RATE 소폭 상향 검토 (0.5→0.6)
