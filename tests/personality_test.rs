...
fn 같은_상황_다른_성격_다른_해석_가능성() {
    ...
    assert!(
        li_avg.a.value() > 0.0 && yu_avg.a.value() < 0.0,
        "무백(A={})은 양수, 교룡(A={})은 음수여야 함",
        li_avg.a.value(),
        yu_avg.a.value()
    );
    assert!(
        li_avg.h.value() > 0.0 && yu_avg.h.value() < 0.0,
        "무백(H={})은 양수, 교룡(H={})은 음수여야 함",
        li_avg.h.value(),
        yu_avg.h.value()
    );
...
fn 성격_modifier_선형_패턴() {
    ...
    assert!(
        (mod_neutral - 1.0).abs() < f32::EPSILON,
        "중립은 변동 없음: {}",
        mod_neutral
    );
}
...
fn npc_json_직렬화_역직렬화() {
    ...
    assert_eq!(
        restored.personality().honesty_humility.sincerity.value(),
        0.8
    );
    assert_eq!(
        restored.personality().agreeableness.forgiveness.value(),
        -0.5
    );
...
fn 수련_절제의_여검객() {
    ...
    assert!(
        avg.h.value() > 0.6,
        "수련의 정직-겸손성은 매우 높아야 함: {}",
        avg.h.value()
    );
    assert!(
        avg.e.intensity() < 0.3,
        "수련의 정서성은 복합적(중립 근처)이어야 함: {}",
        avg.e.value()
    );
    assert!(
        avg.a.value() > 0.4,
