...
fn build_colbert_pool(embedder: &mut BgeM3Embedder, anchors: &[String]) -> Vec<Vec<f32>> {
...
fn dense_vs_colbert_비교() {
    ...
    for case in &cases {
        ...
        println!(
            ...
            case.label, dp, dpm, cp, cpm, da, dam, ca, cam, dd, ddm, cd, cdm,
        );
    }
    ...
    println!("방향 정확도 요약:");
    println!(
        "  P축: Dense {}/{} ({:.0}%)  ColBERT {}/{} ({:.0}%)",
        d_p_ok,
        p_total,
        d_p_ok as f64 / p_total as f64 * 100.0,
        c_p_ok,
        p_total,
        c_p_ok as f64 / p_total as f64 * 100.0
    );
    println!(
        "  A축: Dense {}/{} ({:.0}%)  ColBERT {}/{} ({:.0}%)",
        d_a_ok,
        a_total,
        d_a_ok as f64 / a_total as f64 * 100.0,
        c_a_ok,
        a_total,
        c_a_ok as f64 / a_total as f64 * 100.0
    );
    println!(
        "  D축: Dense {}/{} ({:.0}%)  ColBERT {}/{} ({:.0}%)",
        d_d_ok,
        d_total,
        d_d_ok as f64 / d_total as f64 * 100.0,
        c_d_ok,
        d_total,
        c_d_ok as f64 / d_total as f64 * 100.0
    );
    println!(
        "  합계: Dense {}/{} ({:.0}%)  ColBERT {}/{} ({:.0}%)",
        d_p_ok + d_a_ok + d_d_ok,
